...
const adminAuth = [authenticate, requireRole("admin", "manager")];
...
router.get(
  ...
  ...adminAuth,
  withCache({
    key: (req) => `analytics:sales:${req.query.period ?? "30d"}`,
    ttl: 300,
...
router.get(
  ...
  ...adminAuth,
  withCache({
    key: (req) => `analytics:top-products:${req.query.limit ?? "10"}`,
    ttl: 300,
...
router.get(
  ...
  ...adminAuth,
  withCache({
    key: (req) => `analytics:recent-orders:${req.query.limit ?? "10"}`,
    ttl: 60,
...
router.get(
  ...
  ...adminAuth,
  withCache({
    key: () => "analytics:revenue-by-category",
    ttl: 600,
    tags: ["analytics", "categories"],
  }),
  async (_req, res): Promise<void> => {
    const rows = await db.select({
