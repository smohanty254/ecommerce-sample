...
function buildListKey(req: Parameters<typeof withCache>[0]["key"] extends (r: infer R) => string ? R : never): string {
  const q = req.query;
  const parts = [
    `page=${q.page ?? 1}`,
    `limit=${q.limit ?? 20}`,
    q.categoryId ? `cat=${q.categoryId}` : "",
    q.search ? `q=${q.search}` : "",
    q.sortBy ? `sort=${q.sortBy}` : "",
    q.minPrice ? `min=${q.minPrice}` : "",
    q.maxPrice ? `max=${q.maxPrice}` : "",
    q.inStock ? `inStock=1` : "",
  ].filter(Boolean);
...
router.get(
  ...
  withCache({ key: buildListKey, ttl: 120, tags: ["products"] }),
...
router.post("/products", authenticate, requireRole("admin", "manager"), async (req, res): Promise<void> => {
  ...
  await invalidateTags(["products"]);
  res.status(201).json(await enrichProduct(p));
});

router.get(
  "/products/featured",
  withCache({ key: () => "products:featured", ttl: 300, tags: ["products"] }),
...
router.get(
  "/products/trending",
  withCache({ key: () => "products:trending", ttl: 300, tags: ["products"] }),
  async (_req, res): Promise<void> => {
...
router.get(
  "/products/:id",
  optionalAuthenticate,
  withCache({ key: (req) => `products:${req.params.id}`, ttl: 300, tags: ["products"] }),
  async (req, res): Promise<void> => {
